...
def new_user():
    ...
    if session.query(User).filter_by(username=username).first() is not None:
        ...
        return jsonify({'messsage': 'User already exists'}), 200
...
@app.route('/products/', methods=['GET', 'POST'], strict_slashes=False)
@auth.login_required
def showAllProducts():
...
@app.route('/products/<category>/', strict_slashes=False)
@auth.login_required
